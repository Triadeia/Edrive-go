...
import { ImportLaunchDialog, ListDialog, MemberDialog, SpaceDialog, StructureDeleteDialog, TaskDeleteDialog } from "@/components/tasks/workspace-dialogs";
...
import { moveBahiaDatePreservingTime, isoFromBahiaWallClock } from "@/lib/tasks/workspace-dates";
...
export function TasksWorkspace() {
  ...
  const [spaceFilterId, setSpaceFilterId] = useState<string | "all">("all");
  ...
  const filteredTasks = useMemo(() => {
    ...
    const filteredSpaceListIds = spaceFilterId === "all"
      ? null
      : new Set(workspace.lists.filter((list) => list.spaceId === spaceFilterId).map((list) => list.id));
    return [...workspace.tasks]
      .filter((task) => selectedListId === null || task.listId === selectedListId)
      .filter((task) => filteredSpaceListIds === null || filteredSpaceListIds.has(task.listId))
  ...
  }, [assigneeId, phase, priority, query, selectedListId, spaceFilterId, status, workspace]);
  ...
  if (!workspace) {
    if (taskWorkspace.error) {
      return <section role="alert" className="panel grid min-h-72 place-items-center p-8"><div className="max-w-xl text-center"><AlertCircle className="mx-auto size-8 text-red-300" /><h2 className="mt-3 text-xl font-black">Os dados locais não puderam ser abertos</h2><p className="mt-2 text-sm text-red-200">{taskWorkspace.error}</p><p className="muted mt-3 text-xs">A recuperação remove somente o workspace corrompido deste navegador e recria a estrutura vazia.</p><button type="button" className="danger-button mt-5" onClick={() => void taskWorkspace.recoverCorruptedWorkspace()}>Recuperar workspace local</button></div></section>;
    }
    return <section className="panel grid min-h-72 place-items-center p-8"><div className="text-center"><RefreshCw className="mx-auto size-7 animate-spin text-[var(--primary)]" /><p className="mt-3 font-black">Abrindo dados deste navegador…</p></div></section>;
  }

  const phases = [...new Set(workspace.tasks.map((task) => task.phase))].sort();
  const clearFilters = () => { setQuery(""); setStatus("all"); setPriority("all"); setAssigneeId("all"); setPhase("all"); setSpaceFilterId("all"); };
  ...
  return (
    <section className="tasks-workspace-shell">
      ...
      <div className="min-w-0 space-y-4">
        {taskWorkspace.error ? <div role="alert" className="fixed left-1/2 top-4 z-[140] flex w-[min(92vw,760px)] -translate-x-1/2 items-start gap-3 rounded-lg border border-red-400/40 bg-[#3b1015] p-4 text-sm text-red-50 shadow-2xl"><AlertCircle className="mt-0.5 size-5 shrink-0" /><p className="flex-1">{taskWorkspace.error}</p><button type="button" aria-label="Fechar erro" onClick={taskWorkspace.clearError}><X className="size-4" /></button></div> : null}
        ...
        <header className="panel p-4 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button type="button" className="secondary-button" onClick={() => setImportDialogOpen(true)}><Download className="size-4" /> Importar checklist do lançamento</button>
        ...
        <section className="panel p-3 sm:p-4">
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_repeat(5,minmax(120px,auto))_auto]">
            ...
            <label><span className="sr-only">Filtrar por space</span><select className="form-control" value={spaceFilterId} onChange={(event) => setSpaceFilterId(event.target.value)}><option value="all">Todos os spaces</option>{[...workspace.spaces].sort((a, b) => a.position - b.position).map((space) => <option key={space.id} value={space.id}>{space.name}</option>)}</select></label>
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-3">
            <p className="muted text-xs">Exibindo <strong data-testid="filtered-total" className="text-[var(--foreground)]">{filteredTasks.length}</strong> de {workspace.tasks.length} tarefas</p>
        ...
        <section className="panel overflow-x-auto p-3 sm:p-4">
          ...
          {view === "calendar" ? <TaskCalendarView workspace={workspace} tasks={filteredTasks} onCreate={(date) => setTaskDialog({ task: null, initialDate: date })} onEdit={(task) => setTaskDialog({ task })} onMoveDueDate={(task, date) => { const dueAt = task.dueAt ? moveBahiaDatePreservingTime(task.dueAt, date) : isoFromBahiaWallClock(`${date}T18:00`); void taskWorkspace.updateTask(task.id, { dueAt }); }} /> : null}
        </section>
      ...
      {importDialogOpen ? <ImportLaunchDialog currentTaskCount={workspace.tasks.length} onClose={() => setImportDialogOpen(false)} onConfirm={async () => { const result = await taskWorkspace.importLaunchSeed(); if (result) setImportDialogOpen(false); }} /> : null}
