...
function TaskIdentity({ workspace, task }: { workspace: WorkspaceEnvelope; task: WorkspaceTask }) {
  ...
  return (
    <div className="muted mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs">
      ...
      <span>{task.dueAt ? new Date(task.dueAt).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" }) : "Sem prazo"}</span>
    </div>
  );
}
          ...
          {tasks.map((task) => {
            ...
            return (
              <tr key={task.id} className="border-t border-[var(--border)] align-top hover:bg-[var(--muted)]/50">
                ...
                <td className="p-3 text-sm">{task.dueAt ? new Date(task.dueAt).toLocaleDateString("pt-BR") : "—"}</td>
    ...
    <div className="min-w-[850px] rounded-lg border border-[var(--border)] p-3">
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {days.map((day) => {
          ...
          const dayTasks = props.tasks.filter((task) => task.dueAt?.slice(0, 10) === iso);
...
function CalendarDay(props: { date: string; outside: boolean; tasks: WorkspaceTask[]; onCreate: (date?: string) => void; onEdit: (task: WorkspaceTask) => void; onMoveDueDate: (task: WorkspaceTask, date: string) => void }) {
  ...
  return (
    <div className={`min-h-28 rounded-md border border-[var(--border)] p-1.5 ${props.outside ? "opacity-40" : ""}`}>
      ...
      <div className="space-y-1">{props.tasks.slice(0, 4).map((task) => <div key={task.id} className="flex items-center rounded bg-[var(--muted)]"><button type="button" className="min-w-0 flex-1 truncate p-1 text-left text-[10px] font-bold" onClick={() => props.onEdit(task)}>{task.title}</button><button type="button" className="p-1 text-[var(--primary)]" aria-label={`Mover prazo de ${task.title} para o dia seguinte`} onClick={() => props.onMoveDueDate(task, nextDate)}><ArrowRight className="size-3" /></button></div>)}</div>
    </div>
  );
}
